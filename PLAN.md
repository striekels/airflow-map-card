...
- SVG arrow asset points **north (up)** at rotation 0 and is rotated by `windTo`.
  (The current card gets the same result by rotating a *south*-pointing glyph by
...
*from* the direction the facade faces is what hits the front.
...
Also exposed as an aria-label sentence: *"Wind 10 km/h from the north-east, blowing
front to back through the house."*
...
arrives on *every* state change in the system; re-rendering on all of them is the classic
